...
import { ResourcePerformance, ResourcePerformanceDocument } from './entities/resource-performance.entity';

@Injectable()
export class ResourcePerformanceService {
  constructor(
    @InjectModel(ResourcePerformance.name) private readonly resourcePerformanceModel: Model<ResourcePerformanceDocument>
    ) {}
  ...
  async creates(createResourcePerformanceInputs: [CreateResourcePerformanceInput]) {
    const resp = await this.resourcePerformanceModel.insertMany(createResourcePerformanceInputs);
    console.log(resp);
  ...
  async update(updateResourcePerformanceInput: UpdateResourcePerformanceInput) {
    const resp = await this.resourcePerformanceModel.updateOne(updateResourcePerformanceInput);
    return resp.modifiedCount;
  }

  async remove(id: string) {
    const resp = await this.resourcePerformanceModel.remove({ id });
